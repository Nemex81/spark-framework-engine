...
def test_bootstrap_dry_run_reports_would_copy_without_writing(
    workspace_root: Path,
    bootstrap_tool: Any,
) -> None:
    """dry_run=True: files_copied is populated but nothing is written to disk."""
    result = asyncio.run(bootstrap_tool(dry_run=True))
...
def test_bootstrap_dry_run_reports_files_skipped_when_already_bootstrapped(
    workspace_root: Path,
    bootstrap_tool: Any,
) -> None:
    """dry_run after real bootstrap: files_skipped lists all already-present files."""
    # First real bootstrap
    first = asyncio.run(bootstrap_tool())
    ...
    second = asyncio.run(bootstrap_tool(dry_run=True))
...
def test_bootstrap_new_fields_present_on_fresh_bootstrap(
    workspace_root: Path,
    bootstrap_tool: Any,
) -> None:
...
def test_bootstrap_returns_files_protected_for_non_sentinel_user_modified(
    workspace_root: Path,
    bootstrap_tool: Any,
) -> None:
    ...
    second = asyncio.run(bootstrap_tool())
    assert second["success"] is True
...
def test_bootstrap_force_overwrites_user_modified_non_sentinel_file(
    workspace_root: Path,
    bootstrap_tool: Any,
) -> None:
    ...
    second = asyncio.run(bootstrap_tool(force=True))
    assert second["success"] is True
...
def test_bootstrap_force_overwrites_user_modified_sentinel(
    workspace_root: Path,
    bootstrap_tool: Any,
) -> None:
    ...
    no_force = asyncio.run(bootstrap_tool())
    ...
    with_force = asyncio.run(bootstrap_tool(force=True))
...
def test_bootstrap_files_skipped_populated_after_identical_rerun(
    workspace_root: Path,
    bootstrap_tool: Any,
) -> None:
