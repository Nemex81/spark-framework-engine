...
- Nota sul contratto di ritorno: i client legacy devono continuare a leggere `merged_files`; i client aggiornati dovrebbero preferire `merge_clean` e `merge_conflict` per distinguere merge automatici da conflitti. Liste assenti o non applicabili vengono restituite come `[]`; identificatori singoli non applicabili vengono restituiti come `null` (es. `session_id` quando non è stata creata sessione).
...
**Descrizione**: Bump `ENGINE_VERSION` da 1.9.0 a 2.0.0, aggiornamento CHANGELOG.md con voce versione 2.0.0, aggiornamento contatori tool (da 29 a 33: 29 + 4 nuovi) e risorse nel sorgente e nel log, aggiornamento README e marcatura `docs/ROADMAP-FASE2.md` come completata.
...
class MergeEngine:
    ...
    def diff3_merge(
        ...
        ...
            ...
            # Regola esplicita: se OURS == THEIRS != BASE il motore tratta la sezione come
            # merge "clean" e applica il delta una sola volta (deduplicazione).
...
class MergeSessionManager:
    ...
    def _atomic_write_json(self, path: Path, data: dict[str, Any]) -> None:
        """
                ...
                atomically usando `os.replace()` per garantire comportamento corretto su
                Windows e su altri sistemi. Questa funzione previene file parziali in caso
                di crash o interruzione del processo.
                ...
                with open(tmp_path, "w", encoding="utf-8") as f:
                        ...
                        f.flush(); os.fsync(f.fileno())  # opzionale ma consigliato su file critici
                os.replace(tmp_path, path)

                Note:
                - Il file temporaneo DEVE essere creato nella stessa directory del target
                    per evitare problemi di rename su filesystem diversi (Windows).
                - Se esiste già un `.tmp` all'avvio, il motore lo considera un artefatto
                    di scrittura interrotta e la sessione relativa va riesaminata dal cleanup.
        """
        ...
```
...
- Commento classe `SparkFrameworkEngine`: `# Tools (33)` (29 originali + 4 nuovi: `scf_finalize_update`, `scf_resolve_conflict_ai`, `scf_approve_conflict`, `scf_reject_conflict`).
- Riga ~2407: `_log.info("Tools registered: 33 total")`.
...
- Aggiungere sezione `## Sistema di merge a 3-via` con descrizione delle 3 modalità e workflow di base.
    - Aggiornare contatore tool nella tabella funzionalità (da 29 a 33).
