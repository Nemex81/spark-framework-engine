...
- [x] VALIDATION-REPORT.md presente con verdetto PASS
- [x] Engine v2.4.0 con suite test verde
  (`.venv\Scripts\python.exe -m pytest -q`)
- [x] `SnapshotManager` operativo (riga 1839 di
  `spark-framework-engine.py`)

## Task

- [x] 0.1 Definire helper `_classify_v2_workspace_file(path)` -> str
...
- [x] 0.2 Implementare `MigrationPlanner` interno
      File: `spark-framework-engine.py`
...
- [x] 0.3 Registrare tool `scf_migrate_workspace`
      File: `spark-framework-engine.py`
...
- [x] 0.4 Aggiornare contatore tool registrati nel log boot
      File: `spark-framework-engine.py`
      Cercare stringa "Tools registered" e aggiornare a 36
      (35 esistenti + scf_migrate_workspace).

- [x] 0.5 Test: dry_run su workspace simulato
      File: `tests/test_migrate_workspace.py`
      Usa `tmp_path` di pytest, crea struttura v2.x sintetica.

- [x] 0.6 Test: esecuzione reale con rollback su errore iniettato
      File: `tests/test_migrate_workspace.py`
      Mock di `Path.replace` che lancia OSError, verifica
      `SnapshotManager.rollback()` chiamato.

- [x] 0.7 Test: idempotenza su workspace già migrato
      File: `tests/test_migrate_workspace.py`
      Migrazione ripetuta → migration_plan vuoto, no side-effect.

## Test di accettazione

- [x] Workspace simulato con `agents/code-Agent-Code.md` modificato
      → finisce in `.github/overrides/agents/code-Agent-Code.md`.
- [x] Workspace simulato con `AGENTS.md` non modificato (SHA matches
      pacchetto) → eliminato.
- [x] `.scf-registry-cache.json` → spostato in
      `engine_dir/cache/registry-cache.json`.
- [x] `dry_run=True` non scrive nulla su filesystem.
- [x] `dry_run=False` con error injection → rollback completo.
- [x] Tool count nel log boot aggiornato.
