...
- [x] Fase 0 completata e in main
- [x] Backup di tutti i package-manifest.json prima di toccarli

## Task

- [x] 1.1 Creare `engine-manifest.json` nella root engine
...
- [x] 1.2 Creare agente `spark-welcome.agent.md`
      File:
...
- [x] 1.3 Aggiornare manifest spark-base
...
- [x] 1.4 Aggiornare manifest scf-master-codecrafter
      File: `scf-master-codecrafter/package-manifest.json`
      Identico al 1.3, version → "2.4.0".

- [x] 1.5 Aggiornare manifest scf-pycode-crafter
      File: `scf-pycode-crafter/package-manifest.json`
      Identico al 1.3, version → "2.2.0".

- [x] 1.6 Estendere `EngineInventory` per caricare engine-manifest
...
- [x] 1.7 Test: engine v2.4.0 legge manifest v3.0 senza crash
      File: `tests/test_manifest_compatibility.py`
      Verifica fallback su `files` quando `workspace_files`
      assente, e viceversa.

- [x] 1.8 Test: engine carica engine-manifest.json correttamente
      File: `tests/test_engine_inventory.py` (estendere se esiste)
      Verifica popolamento `mcp_resources` da engine-manifest.

## Test di accettazione

- [x] Tutti i 3 manifest pacchetto a `schema_version: 3.0`.
- [x] Engine v2.4.0 (in stage) avvia correttamente con manifest
      v3.0, log warning fallback solo per `engine_provided_skills`.
- [x] `engine-manifest.json` valido secondo schema definito.
- [x] `.github/agents/spark-welcome.agent.md` esiste con frontmatter
      corretto.
- [x] `EngineInventory` espone metadata di spark-welcome via
      `list_agents()`.
