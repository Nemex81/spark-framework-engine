...
class SparkFrameworkEngine(_V3LifecycleMixin):
    ...
    def register_tools(self) -> None:  # noqa: C901
        ...
        # Pure helpers (no closure capture) are aliased directly to            #
        # spark.boot.install_helpers.  Helpers that capture register_tools()   #
        # scope variables are wrapped as thin shims.                           #
        # ------------------------------------------------------------------ #
        ...
        def _detect_workspace_migration_state() -> dict[str, Any]:
            ...
            return _ih._detect_workspace_migration_state(self._ctx.github_root, manifest)
        ...
        def _classify_install_files(
            ...
            return _ih._classify_install_files(
                package_id, files, manifest, self._ctx.workspace_root, snapshots, file_policies
            )
